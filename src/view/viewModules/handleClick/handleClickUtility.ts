...
const prepareMovingCoordinates = (event: JQuery.ClickEvent, thisView: SliderView): number[] => {
  const cursorXY = [
    ((event.pageX - thisView.$field.position().left) / thisView.step) * thisView.step,
    ((event.pageY - thisView.$field.position().top) / thisView.step) * thisView.step,
  ];
  if (cursorXY[0] < 0) {
    cursorXY[0] = 0;
  }
  if (cursorXY[0] > thisView.fieldSize[0]) {
    console.log(thisView.fieldSize[0]);
